...
# Ensure GRUB background is set, console terminal output is disabled, and themes pointing to deleted paths are commented out
if [ -f /etc/default/grub ]; then
    ...
    if grep -q "^GRUB_BACKGROUND=" /etc/default/grub; then
        sed -i 's|^GRUB_BACKGROUND=.*|GRUB_BACKGROUND="/usr/share/backgrounds/xenolinux/wallpaper.png"|' /etc/default/grub
    else
        echo 'GRUB_BACKGROUND="/usr/share/backgrounds/xenolinux/wallpaper.png"' >> /etc/default/grub
    fi
fi

# Disable duplicate entries generated by standard 10_linux so only 06_xeno_efi generates the boot menu
